...
import { FolderOpen, Upload, FileText, CheckCircle, AlertCircle, Download } from "lucide-react";
...
const Documents = () => {
  const documentTypes = [
    ...
    { type: "POA (Power of Attorney)", required: false, completed: 156, pending: 0 },
  ];

  const recentUploads = [
    { customer: "Rajesh Sharma", document: "PAN Card", date: "25-Jan-2025", status: "Verified" },
    { customer: "Priya Patel", document: "KYC Form", date: "24-Jan-2025", status: "Pending Review" },
    { customer: "Amit Verma", document: "Bank Proof", date: "23-Jan-2025", status: "Verified" },
    { customer: "Sunita Reddy", document: "FATCA", date: "22-Jan-2025", status: "Rejected" },
  ];
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-kotak-navy flex items-center gap-2">
            <FolderOpen className="h-8 w-8 text-kotak-red" />
            Document Management
          </h1>
          <p className="text-muted-foreground">Track compliance documents and KYC status</p>
        </div>
            ...
            {documentTypes.map((doc) => (
              <Card key={doc.type} className="border">
                <CardContent className="pt-6">
                  ...
                  <h3 className="font-semibold text-sm mb-3">{doc.type}</h3>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-sm font-bold text-green-600">{doc.completed}</span>
                    </div>
                    <div className="flex items-center justify-between">
                      <span className="text-xs text-muted-foreground flex items-center gap-1">
                      ...
                      <span className="text-sm font-bold text-orange-600">{doc.pending}</span>
                    </div>
                  </div>
                </CardContent>
              </Card>
